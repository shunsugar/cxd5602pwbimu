#include "imu_node2.hpp"

ImuNode2::ImuNode2() : rclcpp::Node("imu_node2")
{
  ...
  timer_ = this->create_wall_timer(200ms, std::bind(&ImuNode2::controlLoop, this));
}
...
void ImuNode2::openSerial(const std::string & port, const int & baudrate, const float & time_out)
{
...
void ImuNode2::processData(const uint8_t & raw_data)
{
...
void ImuNode2::publishMsg(const std::vector<float> & acceleration,
...
int main(int argc, char *argv[])
{
  ...
  auto node = std::make_shared<ImuNode2>();
  while (rclcpp::ok()) {
    node->controlLoop();
  }
  rclcpp::shutdown();
